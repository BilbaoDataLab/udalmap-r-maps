...
# TODO: check if legally  we can download this file and include it in the repository to avoid downloading
...
# Create lists of id that belong to the different data types: %, €...
col_perc <- sort(c(grep("%",names(udalmap)),grep("‰",names(udalmap))))
col_eur <- grep("€",names(udalmap))
if (language==0)
{ col_ind <- grep("Índice",names(udalmap))
} else if (language==1)
{ col_ind <- grep("indizea",names(udalmap))
} else
{ col_ind <- grep("index",names(udalmap))
}
if (language==0)
{ col_m2 <- grep("Superficie",names(udalmap))
} else if (language==1)
{ col_m2 <- grep("azalera",names(udalmap))
} else
{ col_m2 <- grep("area",names(udalmap))
}

if (!dir.exists("eae"))
  dir.create("eae")

for (i in 1:length(udalmap)) ##change "length(udalmap)" to small number like "3" to test generation of only 3 maps
