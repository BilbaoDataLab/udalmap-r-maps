...
for (i in 1:length(udalmap)) {
  table <- matrix(0,nr=nrow(udalmap[[i]])-1, nc=ncol(udalmap[[i]])-1)
  names <- udalmap[[i]][-1,1]

  years <- c()
  for (j in 2:ncol(udalmap[[i]]))
    years <- c(years,as.character(udalmap[[i]][1,j]))

  for (j in 2:ncol(udalmap[[i]]))
  {
    tmp.col <- gsub("\\.","",udalmap[[i]][,j]) #ex: 1.000.000 -> 1000000
    tmp.col <- gsub(",",".",tmp.col) #ex: 0,53 -> 0.53
    table[,j-1] <- as.numeric(as.character(tmp.col[-1]))
  }

  rownames(table) <-gsub(" \\/ ","\\/", as.character(names))

  if (length(years)>1)
    colnames(table) <- as.vector(years)
  else
    colnames(table) <- as.character(years[[1:length(years)]])

  udalmap[[i]] <- table
}

## Download shape file and unzip
#udalerriak.zip <- "./udalerriak.zip"
#download.file("http://www.euskalgeo.net/sites/euskalgeo.net/files/fitxategi-eranskin/udalerriak_0.zip", udalerriak.zip)
